use candle_core::{D, DType, Device, Result, Tensor, Var, bail};
...
const EPS: f64 = 1e-6;
const PARAM_MAGIC: &[u8; 8] = b"MSNNP001";
...
impl MultiScreenLm {
    ...
    pub fn load_parameters(&self, path: impl AsRef<Path>) -> Result<()> {
        ...
        if &magic != PARAM_MAGIC {
            bail!("invalid parameter file magic");
        }
...
fn tensor_from_dims(values: Vec<f32>, dims: &[usize], device: &Device) -> Result<Tensor> {
    match dims {
